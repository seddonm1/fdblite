...
impl GetOptions {
    ...
    /// Manually specify the key_range
    #[inline]
    pub fn with_key_range(mut self, key_range: KeyRange) -> Self {
...
impl Compare {
    ...
    /// Tests whether a key does not exist
    #[inline]
    pub fn not_exists(key: impl Into<Vec<u8>>) -> Self {
        Self::new(
            key,
            CompareOp::Equal,
            CompareTarget::Version,
...
impl TxnOp {
    pub fn key(&self) -> Vec<u8> {
        match &self.0 {
            PbTxnOp::RequestRange(range_request) => range_request.key.clone(),
            PbTxnOp::RequestPut(put_request) => put_request.key.clone(),
            PbTxnOp::RequestDeleteRange(delete_range_request) => delete_range_request.key.clone(),
            PbTxnOp::RequestTxn(_) => vec![],
        }
    }

    pub fn value(&self) -> Vec<u8> {
        match &self.0 {
            PbTxnOp::RequestPut(put_request) => put_request.value.clone(),
            _ => vec![],
        }
    }
